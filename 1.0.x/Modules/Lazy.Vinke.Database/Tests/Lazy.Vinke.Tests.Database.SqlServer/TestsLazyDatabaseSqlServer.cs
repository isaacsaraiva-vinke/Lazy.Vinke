...
namespace Lazy.Vinke.Tests.Database.SqlServer
{
    ...
    public class TestsLazyDatabaseSqlServer : TestsLazyDatabase
    {
        ...
        public void QueryLike_DataAdapterFill_DbmsDbType_Success()
        {
            ...
            // Act
            DataRow dataRowTest1 = databaseSqlServer.QueryRecord("select * from " + tableName + " where cast(TestId as varchar(2048)) like @TestId", tableName, new Object[] { "%20" });
            DataRow dataRowTest2 = databaseSqlServer.QueryRecord("select * from " + tableName + " where cast(TestId as varchar(2048)) like @TestId", tableName, new Object[] { "21%" });
            DataRow dataRowTest3 = databaseSqlServer.QueryRecord("select * from " + tableName + " where cast(TestId as varchar(2048)) like @TestId", tableName, new Object[] { "%22%" });
