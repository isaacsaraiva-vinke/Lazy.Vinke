    ...
    public static class LazyJsonSerializer
    {
        ...
        public static Type SelectSerializerTypeBuiltIn(Type dataType, LazyJsonSerializerOptions jsonSerializerOptions = null)
        {
            if (dataType != null)
            {
                ...
                {
                    if (dataType.GetGenericTypeDefinition() == typeof(List<>)) return typeof(LazyJsonSerializerList);
                    if (dataType.GetGenericTypeDefinition() == typeof(Dictionary<,>)) return typeof(LazyJsonSerializerDictionary);
                    if (dataType.GetGenericTypeDefinition() == typeof(Queue<>)) return typeof(LazyJsonSerializerQueue);
                    if (dataType.GetGenericTypeDefinition() == typeof(Stack<>)) return typeof(LazyJsonSerializerStack);
                    if (dataType.IsAssignableTo(typeof(ITuple)) == true) return typeof(LazyJsonSerializerTuple);
                }
